...
productRouter.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (product) {
      ...
      res.status(404).send({ message: 'Produto não enontrado' })
    }
  })
)
...
productRouter.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (product) {
      ...
      res.send({ message: 'Produto excluido', product: deleteProduct })
    } else {
      res.status(404).send({ message: 'Produto não enontrado' })
    }
  })
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (product) {
      if (product.reviews.find((x) => x.name === req.user.name)) {
        res.status(400).send({ message: 'Você já fez uma review' })
        return
      }
      const review = {
        name: req.user.name,
